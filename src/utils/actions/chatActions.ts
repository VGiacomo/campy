...
export const createChat = async (
  ...
  try {
    const newChat = await addDoc(collection(dbFirestore, "chats"), newChatData);

    const chatUsers = newChatData.users; // array of userIds(chat participants)
    for (let userId of chatUsers) {
      const userChatsRef = doc(dbFirestore, "userChats", userId);
...
export const getUserChats = async (userId: string) => {
  try {
    ...
    if (docSnap.exists()) {
      ...
      console.log("No userChats correspond to this ID!");
    }
  } catch (error) {
    console.log(error);
  }
